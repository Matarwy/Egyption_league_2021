    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() == add_player_button) {
            ...
            int R_index = referee_i.getSelectedIndex(),
                S_index = stadium_i.getSelectedIndex(),
                T1_index = team1_i.getSelectedIndex(),
                T2_index = team2_i.getSelectedIndex(),
                date1=Integer.parseInt(Date.substring(0, Date.indexOf('/'))),
                date2=Integer.parseInt(Date.substring(Date.indexOf('/')+1,Date.lastIndexOf('/'))),
                date3=Integer.parseInt(Date.substring(Date.lastIndexOf('/')+1)),
                time1=Integer.parseInt(Date.substring(0, Time.indexOf(':'))),
                time2=Integer.parseInt(Date.substring(Time.indexOf(':')+1));
                
            if ((T1_index == T2_index)
                    || (Date.equals(""))
                    || (Time.equals(""))
                    || (!((Date.indexOf('/') == 2)||(Date.indexOf('/') == 1)))//!=22/10/2021
                    || (!((Date.lastIndexOf('/') == 5)||(Date.lastIndexOf('/') == 3)||(Date.lastIndexOf('/') == 4)))
                    || (!((Time.indexOf(':') == 2)||(Time.indexOf(':') == 1)))//06:30
                    || (Date.length() > 10)
                    || (Time.length() > 5)
                    || (date1>31||date1<=0)
                    || (date2>12||date2<=0)
                    || (date3>2022||date3<2021)
                    || (time1>24||time1<=0)
                    || (time2>=60||time2<=0)) {
                ...
                match_valid_inf.setText(ex.getMessage());
            }
...
//edit panels----------------------------------------------------------------------------
        ...
        if (e.getSource() == editButton1) {
            ...
            if ((T1_index == T2_index)
                    ...
                    || (Time.equals(""))
                    || (!((Date.indexOf('/') == 2)||(Date.indexOf('/') == 1)))//!=22/10/2021
                    || (!((Date.indexOf('/', 3) == 5)||(Date.indexOf('/', 3) == 3)||(Date.indexOf('/', 4) == 3)))
                    || (!((Time.indexOf(':') == 2)||(Time.indexOf(':') == 1)))//06:30
                    || (Date.length() > 10)
                    || (Time.length() > 5)) {
            ...
            try {
                ...
                if ((P_index >= admin.league.teamList.get(T_index).playerList.size())
                    ...
                    for (int i = 0; i < admin.league.teamList.size(); i++) {
                        for (int j = 0; j < admin.league.teamList.get(i).playerList.size(); j++) {
                            if ((i == T_index)&& (j == P_index)) {
                                PlayerTableModel.removeRow(j+player_size);
                                num_players--;
                                TeamTableModel.setValueAt(num_players, T_index, 2);
                            }
                        }
                        player_size+=admin.league.teamList.get(i).playerList.size();
                    }
